...
extern "C"
{
    ...
    typedef scf_status (*scf_cipher_init_fn)(
        void **state,
        scf_const_buffer key,
        scf_const_buffer nonce);
    typedef scf_status (*scf_cipher_crypt_fn)(
        void *state,
    ...
    SCF_API scf_status scf_cipher_context_create(
        const scf_cipher_provider *provider,
        scf_const_buffer key,
        scf_const_buffer nonce,
        scf_cipher_context **context);
    SCF_API scf_status
    scf_cipher_encrypt(scf_cipher_context *context,
                       scf_const_buffer input,
                       scf_buffer output);
    SCF_API scf_status
    scf_cipher_decrypt(scf_cipher_context *context,
                       scf_const_buffer input,
                       scf_buffer output);
    SCF_API scf_status
    scf_cipher_reset(scf_cipher_context *context);
    SCF_API void
    scf_cipher_context_destroy(scf_cipher_context *context);
