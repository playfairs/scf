...
#define SCF_PROVIDER_NAME_MAX 64
    ...
    typedef scf_status (*scf_provider_context_create_fn)(
        scf_const_buffer parameters,
        void **state);
    typedef void (*scf_provider_context_destroy_fn)(
        void *state);
    ...
    typedef struct scf_provider_registry
        scf_provider_registry;
    typedef struct scf_provider_context
        scf_provider_context;

    SCF_API scf_status scf_provider_registry_create(
        scf_provider_registry **registry);
    SCF_API scf_status scf_provider_register(
        scf_provider_registry *registry,
    ...
    scf_provider_unregister(scf_provider_registry *registry,
                            scf_provider_type type,
                            uint32_t identifier);
    SCF_API scf_status scf_provider_lookup(
        const scf_provider_registry *registry,
        scf_provider_type type,
        uint32_t identifier,
        scf_provider_info *info);
    SCF_API scf_status scf_provider_context_create(
        scf_provider_registry *registry,
        scf_provider_type type,
        uint32_t identifier,
        scf_const_buffer parameters,
        scf_provider_context **context);
    SCF_API scf_status scf_provider_context_info(
        const scf_provider_context *context,
        scf_provider_info *info);
    SCF_API void scf_provider_context_destroy(
        scf_provider_context *context);
    SCF_API scf_status scf_provider_registry_destroy(
        scf_provider_registry *registry);
