...
int scf_unit_provider(void)
{
    ...
    scf_provider_context_destroy(context);
    if (destroyed != 2 || scf_provider_registry_destroy(registry) != SCF_STATUS_SUCCESS) {
        return 1;
    }
    return 0;
}