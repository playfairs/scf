...
static scf_status
provider_create(scf_const_buffer parameters, void **state)
{
...
int scf_unit_provider(void)
{
    ...
    const scf_provider_descriptor descriptor = {
        ...
        provider_create,
        provider_destroy};
    const scf_provider_descriptor invalid = {
        SCF_PROVIDER_HASH,
        2,
        "invalid",
        0,
        provider_create,
        provider_destroy};
    const scf_provider_descriptor missing_name = {
        SCF_PROVIDER_HASH,
        3,
        NULL,
        SCF_PROVIDER_CAP_CONTEXT,
        provider_create,
        provider_destroy};
    const scf_provider_descriptor long_name = {
        SCF_PROVIDER_HASH,
        4,
    ...
    created = 0;
    destroyed = 0;
    if (scf_provider_registry_create(&registry)
            != SCF_STATUS_SUCCESS
        || scf_provider_register(registry, &descriptor)
               != SCF_STATUS_SUCCESS
        || scf_provider_register(registry, &descriptor)
               != SCF_STATUS_DUPLICATE
        || scf_provider_register(registry, &invalid)
               != SCF_STATUS_INVALID_ARGUMENT
        || scf_provider_register(registry, &missing_name)
               != SCF_STATUS_INVALID_ARGUMENT
        || scf_provider_register(registry, &long_name)
               != SCF_STATUS_NAME_TOO_LONG
        || scf_provider_lookup(registry,
                               SCF_PROVIDER_HASH,
                               1,
                               &info)
               != SCF_STATUS_SUCCESS
        || strcmp(info.name, "dummy") != 0
        || info.capabilities != SCF_PROVIDER_CAP_CONTEXT
        || scf_provider_lookup(registry,
                               SCF_PROVIDER_HASH,
                               9,
                               &info)
               != SCF_STATUS_NOT_FOUND
        || scf_provider_context_create(
               registry,
               SCF_PROVIDER_HASH,
               9,
        ...
        || scf_provider_context_create(
               ...
               != SCF_STATUS_SUCCESS
        || created != 1
        || scf_provider_context_info(context, &info)
               != SCF_STATUS_SUCCESS
        || scf_provider_unregister(registry,
                                   SCF_PROVIDER_HASH,
                                   1)
               != SCF_STATUS_SUCCESS
        || scf_provider_lookup(registry,
