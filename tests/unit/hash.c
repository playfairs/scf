...
static scf_status hash_init(void **state)
{
	*state = calloc(1, sizeof(hash_state));
	return *state == NULL ? SCF_STATUS_ALLOCATION_FAILED : SCF_STATUS_SUCCESS;
}

static scf_status hash_update(void *state, scf_const_buffer input)
{
...
static scf_status hash_final(void *state, scf_buffer output, scf_size *written)
{
	hash_state *hash = state;
...
{
	scf_hash_provider provider = {"test", 8, 8, hash_init, hash_update, hash_final, hash_reset, hash_destroy};
	...
	if (scf_hash_context_create(&provider, &context) != SCF_STATUS_SUCCESS || scf_hash_update(context, (scf_const_buffer){input, sizeof(input)}) != SCF_STATUS_SUCCESS || scf_hash_final(context, (scf_buffer){output, sizeof(output)}, &written) != SCF_STATUS_SUCCESS || written != sizeof(expected)) {
