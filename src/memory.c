...
static int scf_secure_add_overflow(scf_size left, scf_size right, scf_size *result)
{
...
static int scf_secure_align_up(scf_size value, scf_size alignment, scf_size *result)
{
	...
	return scf_secure_add_overflow(value, alignment - remainder, result);
}

scf_status scf_secure_allocate(scf_size size, scf_size alignment, scf_secure_allocation **allocation)
{
	...
	*allocation = NULL;
	if (alignment != 0 && !scf_secure_alignment_valid(alignment)) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
	...
	result->raw = NULL;
	result->size = size;
	result->alignment = alignment == 0 ? sizeof(void *) : alignment;
	if (size == 0) {
		*allocation = result;
		return SCF_STATUS_SUCCESS;
	}
	if (scf_secure_add_overflow(size, result->alignment - 1, &raw_size) || scf_secure_add_overflow(raw_size, sizeof(void *), &raw_size)) {
	...
	if (scf_secure_align_up((scf_size)(uintptr_t)(result->raw + sizeof(void *)), result->alignment, &aligned_offset)) {
...
scf_status scf_secure_data(scf_secure_allocation *allocation, scf_buffer *buffer)
{
...
scf_status scf_secure_size(const scf_secure_allocation *allocation, scf_size *size)
{
...
scf_status scf_secure_clear(scf_secure_allocation *allocation)
{
	if (allocation == NULL) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
