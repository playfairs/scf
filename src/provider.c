...
{
	return type >= SCF_PROVIDER_HASH && type <= SCF_PROVIDER_FORMAT;
}

static void scf_provider_lock(scf_provider_registry *registry)
{
	while (atomic_flag_test_and_set_explicit(&registry->lock, memory_order_acquire)) {
	}
}

static void scf_provider_unlock(scf_provider_registry *registry)
{
	atomic_flag_clear_explicit(&registry->lock, memory_order_release);
}

static scf_status scf_provider_descriptor_validate(const scf_provider_descriptor *descriptor)
{
	scf_size length;

	if (descriptor == NULL || !scf_provider_type_valid(descriptor->type) || descriptor->identifier == 0 || descriptor->name == NULL || descriptor->name[0] == '\0' || descriptor->context_create == NULL || descriptor->context_destroy == NULL || (descriptor->capabilities & SCF_PROVIDER_CAP_CONTEXT) == 0) {
...
static scf_provider_entry *scf_provider_find_locked(scf_provider_registry *registry, scf_provider_type type, uint32_t identifier)
{
	for (scf_provider_entry *entry = registry->entries; entry != NULL; entry = entry->next) {
		if (!entry->removed && entry->descriptor.type == type && entry->descriptor.identifier == identifier) {
			return entry;
		}
	}
	return NULL;
...
static void scf_provider_info_copy(const scf_provider_entry *entry, scf_provider_info *info)
{
...
static void scf_provider_remove_entry_locked(scf_provider_registry *registry, scf_provider_entry *target)
{
...
scf_status scf_provider_registry_create(scf_provider_registry **registry)
{
	if (registry == NULL) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
...
scf_status scf_provider_register(scf_provider_registry *registry, const scf_provider_descriptor *descriptor)
{
	scf_provider_entry *entry;
	scf_status status;
	...
	if (status != SCF_STATUS_SUCCESS || registry == NULL) {
		return registry == NULL ? SCF_STATUS_INVALID_ARGUMENT : status;
	}
	scf_provider_lock(registry);
	if (scf_provider_find_locked(registry, descriptor->type, descriptor->identifier) != NULL) {
	...
	memcpy(entry->name, descriptor->name, strlen(descriptor->name) + 1);
...
scf_status scf_provider_unregister(scf_provider_registry *registry, scf_provider_type type, uint32_t identifier)
{
	scf_provider_entry *entry;

	if (registry == NULL || !scf_provider_type_valid(type) || identifier == 0) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
	scf_provider_lock(registry);
	entry = scf_provider_find_locked(registry, type, identifier);
	if (entry == NULL) {
		scf_provider_unlock(registry);
...
scf_status scf_provider_lookup(const scf_provider_registry *registry, scf_provider_type type, uint32_t identifier, scf_provider_info *info)
{
	scf_provider_entry *entry;

	if (registry == NULL || info == NULL || !scf_provider_type_valid(type) || identifier == 0) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
	scf_provider_lock((scf_provider_registry *)registry);
	entry = scf_provider_find_locked((scf_provider_registry *)registry, type, identifier);
	if (entry == NULL) {
...
scf_status scf_provider_context_create(scf_provider_registry *registry, scf_provider_type type, uint32_t identifier, scf_const_buffer parameters, scf_provider_context **context)
{
	scf_provider_entry *entry;
	scf_status status;
...
scf_status scf_provider_context_info(const scf_provider_context *context, scf_provider_info *info)
{
	if (context == NULL || info == NULL || context->entry == NULL) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
...
void scf_provider_context_destroy(scf_provider_context *context)
{
	scf_provider_entry *entry;
	scf_provider_registry *registry;
...
scf_status scf_provider_registry_destroy(scf_provider_registry *registry)
{
	if (registry == NULL) {
		return SCF_STATUS_INVALID_ARGUMENT;
	}
