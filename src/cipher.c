...
scf_status scf_cipher_context_create(
    const scf_cipher_provider *provider,
    scf_const_buffer key,
    scf_const_buffer nonce,
    scf_cipher_context **context)
{
    if (provider == NULL || context == NULL
        ...
        || !scf_internal_buffer_valid(key)
        || !scf_internal_buffer_valid(nonce))
    {
        return SCF_STATUS_INVALID_ARGUMENT;
    }
    if (key.size != provider->key_size
        || nonce.size != provider->nonce_size)
    {
        return SCF_STATUS_INVALID_ARGUMENT;
    }
    ...
    *context = scf_internal_alloc(sizeof(**context));
    ...
    if ((key.size > 0 && (*context)->key == NULL)
        || (nonce.size > 0 && (*context)->nonce == NULL))
    {
    ...
    scf_internal_copy((*context)->nonce,
                      nonce.data,
                      nonce.size);
    if (provider->init(
            &(*context)->state,
            (scf_const_buffer){(*context)->key, key.size},
            (scf_const_buffer){(*context)->nonce,
                               nonce.size})
        != SCF_STATUS_SUCCESS)
    {
        scf_cipher_context_destroy(*context);
...
static scf_status
scf_cipher_crypt(scf_cipher_context *context,
                 scf_const_buffer input,
                 scf_buffer output,
                 scf_cipher_crypt_fn operation)
{
...
scf_status scf_cipher_encrypt(scf_cipher_context *context,
                              scf_const_buffer input,
                              scf_buffer output)
{
    return scf_cipher_crypt(
        context,
        input,
        output,
        context == NULL ? NULL
                        : context->provider->encrypt);
}

scf_status scf_cipher_decrypt(scf_cipher_context *context,
                              scf_const_buffer input,
                              scf_buffer output)
{
    return scf_cipher_crypt(
...
void scf_cipher_context_destroy(scf_cipher_context *context)
{
    if (context != NULL)
    {
        ...
        scf_internal_clear(context->key,
                           context->provider->key_size);
        scf_internal_clear(context->nonce,
                           context->provider->nonce_size);
