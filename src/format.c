...
static void scf_format_write_u32(scf_byte *output,
                                 uint32_t value)
{
...
scf_status
scf_format_validate(const scf_format_header *header)
{
    scf_byte empty_magic[sizeof(header->magic)] = {0};

    if (header == NULL
        || scf_internal_equal(header->magic,
                              empty_magic,
                              sizeof(header->magic))
        || header->version == 0 || header->type == 0
        || header->payload_size > SIZE_MAX)
    {
...
scf_status
scf_format_context_create(scf_format_context **context)
{
    if (context == NULL)
    {
...
scf_status
scf_format_header_set(scf_format_context *context,
                      const scf_format_header *header)
{
...
scf_status
scf_format_header_get(const scf_format_context *context,
                      scf_format_header *header)
{
    if (context == NULL || header == NULL)
    {
...
scf_status
scf_format_serialize(const scf_format_header *header,
                     scf_buffer output,
                     scf_size *written)
{
    uint64_t payload_size;

    if (header == NULL || written == NULL
    ...
    scf_internal_copy(output.data,
                      header->magic,
                      sizeof(header->magic));
    scf_format_write_u32(output.data + 4, header->version);
    scf_format_write_u32(output.data + 8, header->type);
    payload_size =
        scf_internal_byteswap64(header->payload_size);
    scf_internal_copy(output.data + 12,
                      &payload_size,
                      sizeof(payload_size));
...
scf_status scf_format_deserialize(scf_const_buffer input,
                                  scf_format_header *header)
{
    uint64_t payload_size;
    ...
    scf_internal_copy(header->magic,
                      input.data,
                      sizeof(header->magic));
    header->version = scf_format_read_u32(input.data + 4);
    header->type = scf_format_read_u32(input.data + 8);
    scf_internal_copy(&payload_size,
                      input.data + 12,
                      sizeof(payload_size));
    header->payload_size =
        scf_internal_byteswap64(payload_size);
