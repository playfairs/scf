...
scf_status scf_key_create(scf_const_buffer material,
                          scf_key **key)
{
    if (key == NULL || !scf_internal_buffer_valid(material)
        || material.size == 0)
    {
    ...
    *key = scf_internal_alloc(sizeof(**key));
    ...
    scf_status status =
        scf_secure_allocate(material.size,
                            0,
                            &(*key)->material);
    ...
    scf_internal_copy(destination.data,
                      material.data,
                      material.size);
...
scf_status scf_key_copy(const scf_key *source,
                        scf_key **copy)
{
    if (source == NULL || copy == NULL
        || source->state != SCF_KEY_INITIALIZED)
    {
        return SCF_STATUS_INVALID_ARGUMENT;
    }
    scf_const_buffer material;
    if (scf_key_data(source, &material)
        != SCF_STATUS_SUCCESS)
    {
...
scf_status scf_key_data(const scf_key *key,
                        scf_const_buffer *material)
{
    ...
    scf_buffer data;
    if (scf_secure_data(key->material, &data)
        != SCF_STATUS_SUCCESS)
    {
        return SCF_STATUS_INVALID_STATE;
    }
...
scf_size scf_key_size(const scf_key *key)
{
    scf_size size;
    return key == NULL
                   || scf_secure_size(key->material, &size)
                          != SCF_STATUS_SUCCESS
               ? 0
               : size;
}
